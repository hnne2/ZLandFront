...
const { data, error, status } = await useFetch('/api/catalog/data')
...
useHead(() => ({
	title: category.value?.category?.name ? `${category.value.category.name}` : 'Загрузка...'
}))
		...
		<div class="container">
			...
			<div v-else>
				...
				<ul class="catalog__list">
					<li v-for="product in category?.products" :key="product.id" class="catalog__item">
						<div class="card">
							<div class="card__img">
							...
							<div class="card__parameters">
								<div v-for="parametr in product.parameter" :key="parametr.name" class="card__parameters-item">
									<div class="card__parameters-info p3-medium">
										<span>{{ parametr.name }}</span>
										<span>{{ parametr.value }}</span>
									</div>
									<div class="card__parameters-indicator" :data-value="parametr.value">
										<span></span>
										<span></span>
										<span></span>
									</div>
...
.card {
	...
	&__parameters-item {
		...
		&:nth-child(1) .card__parameters-indicator span::before {
			background-color: $color-pink;
		}

		&:nth-child(2) .card__parameters-indicator span::before {
			background-color: $color-green;
		}

		&:nth-child(3) .card__parameters-indicator span::before {
