				...
				<div class="proof__form">
					<div class="proof__form-info">
						<p class="proof__description p1-medium">
							Я подтверждаю, что мне исполнилось 18 или более лет, и я являюсь потребителем табака или иной содержащей
							никотин продукции
						</p>
						<p class="proof__subtitle">Укажите дату вашего рождения</p>
					</div>
					<Form v-slot="{ meta: formMeta }" class="form" :validation-schema="schema" @submit="handleSubmit">
						<Field v-slot="{ field, meta, errorMessage }" v-model="form.date" name="date" validate-on-input>
							<div class="input">
								<input
									v-maska="'##.##.####'"
									v-bind="field"
								...
								<p v-if="errorMessage" class="input__msg error">{{ errorMessage }}</p>
							</div>
						</Field>
						<button :disabled="!formMeta.valid || isLoading" class="btn btn--light" type="submit">Подтвердить</button>
					</Form>
				</div>
				<p class="proof__footer p3-medium">
					Этот сайт содержит информацию о бездымных продуктах, предназначенных только для лиц старше 18 лет
				</p>
			</div>
		</div>
	</div>
